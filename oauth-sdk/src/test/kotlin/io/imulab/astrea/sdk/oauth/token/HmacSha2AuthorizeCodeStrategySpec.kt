...
object HmacSha2AuthorizeCodeStrategySpec : Spek({

    given("A configured strategy") {
        val strategy = HmacSha2AuthorizeCodeStrategy(
        ...
        `when`("asked to generate a token") {
            val token = runBlocking { strategy.generateCode(mock()) }
            ...
            then("should be able to verify the same token") {
                assertThatCode { runBlocking { strategy.verifyCode(token, mock()) } }.doesNotThrowAnyException()
            }
