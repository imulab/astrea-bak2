package io.imulab.astrea.service.login
...
private val rootLogger = LoggerFactory.getLogger("io.imulab.astrea.service.login.Main")

fun main() {
    val vertx = Vertx.vertx(VertxOptions())

    vertx.deployVerticle(LoginVerticle(ConfigFactory.load())) { ar ->
        if (ar.succeeded()) {
...
class LoginVerticle(private val appConfig: Config) : AbstractVerticle() {
    ...
    override fun start() {
        val router = Router.router(vertx).apply {
            get("/").handler { rc ->
                val loginToken = JsonWebSignature().apply {
                ...
                val url = HttpUrl.parse(appConfig.getString("authorizeProxy.url"))!!
                    ...
                    .apply { addQueryParameter("login_token", loginToken) }
                    .build()
                    .toString()
